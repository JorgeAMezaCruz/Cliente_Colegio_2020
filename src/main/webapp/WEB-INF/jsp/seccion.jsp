...
<body id="page-top">
		...
		<div class="container body-content">
			...
			<div class="card shadow-lg">
				<div class="card-header">
					<h3 class="m-0 font-weight-bold text-dark">Listado de
						Secciones</h3>
					<br>

					<button type="button" class="btn btn-primary" data-toggle="modal"
						data-target="#modalProgramador">Nueva Seccion</button>
						...
						<tbody>
							...
							<c:forEach items="${requestScope.dataSecciones}" var="s">
								<tr>
									<td>${s.idSeccion}</td>
									<td>${s.nomSeccion}</td>
									<td>${s.horario.descripcion}</td>

									<td>
										<button type='button' data-toggle='modal'
											onclick="editar('${s.idSeccion}','${s.nomSeccion}','${s.horario.descripcion}');"
				...
				<div class="modal-dialog">
					<div class="modal-content">
						<div class="modal-header">
							<h5 class="modal-title" id="idTitulo">Registrar nueva
								sección</h5>
						</div>
						<div class="modal-body">
							<form id="frmProgramador" action="saveSeccion" method="post"
								data-toggle="validator" role="form">
								<div hidden class="form-group">
									<label class="col-lg-3 control-label" for="id_seccion">Codigo
										de Seccion</label>
									<div class="col-lg-5">
										<input class="form-control" id="id_seccion"
											readonly="readonly" name="id_seccion" type="text" value="0" />
									</div>
								</div>
								<div class="form-group">
									<label for="exampleInputEmail1">Nombre de seccion</label> <input
										type="text" class="form-control" name="nombre_seccion"
										id="nombre_seccion">
								</div>
								<div class="form-group">
									<label>Horario</label> <select class="form-control"
										id="horario" name="horario" required>
										<option value="">[Seleccione]</option>
										<c:forEach items="${requestScope.dataHorarios}" var="h">
											<option value="${h.idHorario}">${h.descripcion}</option>
										</c:forEach>
									</select>
								</div>
					...
					<div class="modal-content">
						...
						<div class="modal-body">
							<form id="id_form_actualiza" accept-charset="UTF-8"
								action="saveSeccion" class="form-horizontal" method="post"
								data-toggle="validator">
								<div class="form-group">
									<label for="id_seccion">Codigo de Seccion</label> <input
										class="form-control" id="id_seccion" readonly="readonly"
										name="id_seccion" type="text" maxlength="8" />
								</div>
								<div class="form-group">
									<label for="exampleInputEmail1">Nombre de Seccion</label> <input
										type="text" class="form-control" name="nombre_seccion"
										id="act_nombre_seccion">
								</div>

								<div class="form-group">
									<label for="exampleInputEmail1">Horario</label> <select
										class="form-control" id="horario" name="horario" required>
										<c:forEach items="${requestScope.dataHorarios}" var="h">
											<option value="${h.idHorario}">${h.descripcion}</option>
										</c:forEach>
									</select>
	...
	<script type="text/javascript">
		function editar(id, nom, h) {
			$('input[id=id_seccion]').val(id);
			$('input[id=act_nombre_seccion]').val(nom);
			$('select[id=act_horario]').val(h);
									...
									.bootstrapValidator(
											{
												fields : {
													nombre_seccion : {
														validators : {
															notEmpty : {
																message : 'El nombre de seccion es obligatorio'
															},
															regexp : {
																regexp : /^[a-zA-Z\s\ñ\Ñ\á\é]{4,4}$/,
																message : 'Campo seccion solo letras min 4 hasta max 4 letras'
															}
														}
													},
													horario : {
